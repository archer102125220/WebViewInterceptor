...
3. **攔截死角 / 失效測試**：
    ...
    * **表單跳轉 (`<form>`)**：測試發現無論是既有或動態建立的 form 表單，執行 `submit()` 皆能完美繞過包含極端超時在內的任何非同步封殺限制。
      * **GET**：雙平台皆能成功跳轉並正常攔截參數，為**目前最完美的純網頁繞過方案**。
      * **POST**：Android 原生無法攔截 POST 請求（直接跳轉穿透），iOS 雖成功攔截但 Body 會因 IPC 限制而永遠為 nil。
...
5. 保持伺服器運行，重新編譯並啟動 Android 或 iOS App，App 會自動讀取該 IP，您會在首頁最上方看到專屬的 **302 延遲測試區塊**，點擊即可實測效果。
