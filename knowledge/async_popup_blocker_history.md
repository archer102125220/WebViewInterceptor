...
2. **Microtask (微任務) 與 Promise 的演進史**：
   ...
   - **【標準化與 Fetch 放行】2020 年 (Safari 14 / iOS 14 左右)**：過去雖然純 Promise 能過關，但如果是 `fetch` 等涉及網路請求的 Promise 則往往會丟失 Token。WebKit 在 2020 年 8 月 (WebKit Bugzilla #215014) 正式將「透過 Promise 轉送使用者手勢」的標準化機制預設開啟，這也是 WebKit 邁向標準化追蹤 (Bugzilla #225559) 的重要里程碑，讓包含 `fetch` 等非同步請求也能在一定條件下合法繼承 Token。
   - **【薛丁格狀態】近年 (iOS 15 以後) 與 In-App Browser**：儘管底層引擎逐步標準化了 Promise 的 Token 傳遞，Apple 同時也大幅強化了隱私權與防彈窗濫用機制 (如 WebKit Bugzilla #264444 提及的相關手勢限制與演進)。在很多實際場景中 (特別是社群軟體的內建 WebView / In-App Browser，或開啟了進階防護)，一旦非同步耗時過長，微任務的 Token 仍會被無情沒收，導致前端開發者覺得彈窗機制「時好時壞」。
...
- 📖 [WebKit Bugzilla #215014：Move user gesture propagation over promise behind a feature flag (2020 年正式將 Promise 轉送機制預設開啟)](https://bugs.webkit.org/show_bug.cgi?id=215014)
- 📖 [WebKit Bugzilla #264444：關於移除或修改某些手勢限制的討論與近期演進](https://bugs.webkit.org/show_bug.cgi?id=264444)
...
- 📖 [StackOverflow：Safari `window.open` async workaround (業界針對 Safari 非同步彈窗的標準實務解法：先開空白視窗再換 URL)](https://stackoverflow.com/questions/20696041/window-openurl-blank-not-working-on-imac-safari)
