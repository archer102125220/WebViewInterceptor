...
本文件將解析在不同 Android 環境下，前端使用 `window.open` 或非同步跳轉時可能遭遇的落差與限制。
...
- **測試結果通用**：在這些設備上，Chromium UAv2 的「5 秒鐘憑證寬限期」機制會高度一致地運作。只要非同步 API 延遲在 5 秒內，`window.open` 皆能順利觸發。
...
- **自定義防禦規則**：X5 內核的資安政策與 `window.open` 攔截邏輯與 Chromium 截然不同，通常更為嚴格且包含未公開的黑盒限制。
- **機制失效**：在這種環境下，Chromium 的 5 秒寬限期並不適用。不僅非同步彈窗會失效，甚至在某些情境下，連同步的 `window.open` 也會遭到特製規則無情封殺。
...
如果網頁是透過 Facebook、LINE 等第三方 Android App 的內建瀏覽器開啟，只要該 App 的原生開發者沒有在 `WebChromeClient` 中實作 `onCreateWindow`（或者實作了但刻意不處理），那麼前端發出的 `window.open` 請求同樣會毫無反應。這與底層是否具備 Chromium 5 秒寬限期無關，純粹是應用層的刻意封閉。

---

## 結論與對策

雖然搭載 GMS 的現代 Android 手機在 WebView 行為上比較一致，但只要專案的目標受眾包含**會使用微信 (WeChat) 開啟網頁的用戶**，或是**使用無 GMS 設備 (中國大陸市場) 的用戶**，`window.open` 的穩定性依然是一場災難。
